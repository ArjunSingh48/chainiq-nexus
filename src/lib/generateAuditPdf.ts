...
export function generateAuditPdf(data: AuditData): void {
  ...
  if (data.chatMessages.length > 0) {
    for (const msg of data.chatMessages) {
      ...
      doc.setTextColor(...WHITE);
      const lines = doc.splitTextToSize(msg.text, 160);
      for (const line of lines) {
        if (y > 280) { doc.addPage(); doc.setFillColor(...DARK); doc.rect(0, 0, 210, 297, 'F'); y = 20; }
        doc.text(line, 30, y);
        y += 4.5;
      }
      y += 2;
  ...
  // ── 4. SUPPLIER ANALYSIS ──
  if (y > 220) { doc.addPage(); doc.setFillColor(...DARK); doc.rect(0, 0, 210, 297, 'F'); y = 20; }
  y = addSectionHeader(doc, '4. SUPPLIER ANALYSIS', y);
  y = addField(doc, 'Total Evaluated', data.suppliers.length.toString(), y);
  y = addField(doc, 'Shortlisted (Top 10)', data.top10.length.toString(), y);
  y += 2;
  ...
  if (data.top10.length > 0) {
    autoTable(doc, {
      ...
      theme: 'grid',
      styles: { fontSize: 7, textColor: WHITE, fillColor: [30, 30, 45], lineColor: [60, 60, 80], lineWidth: 0.2 },
  ...
  for (const n of rejected) { y = addWrappedText(doc, `✗ ${n.message}`, 18, y, 168); }
  y += 4;

  // ── 9. FINAL SUMMARY ──
  if (y > 250) { doc.addPage(); doc.setFillColor(...DARK); doc.rect(0, 0, 210, 297, 'F'); y = 20; }
